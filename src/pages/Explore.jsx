  ...
  return (
    <div className="bg-[#0b1425] border border-cyan-500/10 rounded-lg overflow-hidden cursor-pointer hover:-translate-y-2 transition-all duration-300 hover:border-cyan-400/30 group">
      ...
      <div className="p-4">
        ...
        <div className="text-white text-sm font-bold mb-1 truncate">{clip.title || 'Untitled Clip'}</div>
        <div className="text-slate-500 text-xs mb-3 truncate">🎵 {clip.music}</div>
        <div className="flex justify-between items-center">
          <span className="text-slate-500 text-xs">👁 {clip.views || 0}</span>
          <div className="flex gap-2">
            <button onClick={() => onLike(clip)} className={`text-xs transition-all duration-200 ${liked ? 'text-pink-400' : 'text-slate-500 hover:text-pink-400'}`}>
              {liked ? '❤️' : '🤍'} {clip.likes || 0}
            </button>
            <button onClick={() => onComment(clip)} className="text-xs text-slate-500 hover:text-cyan-400 transition-all duration-200">💬</button>
            <button onClick={() => onShare(clip)} className="text-xs text-slate-500 hover:text-cyan-400 transition-all duration-200">🔗</button>
          </div>
        </div>
      </div>
    </div>
...
function Explore() {
  ...
  async function fetchClips() {
    const { data, error } = await supabase
      .from('clips').select('*').order('created_at', { ascending: false })
    if (!error && data) {
      setClips(data.map(clip => ({
        ...clip,
  ...
  const loadMore = useCallback(() => {
    ...
    setLoadingMore(true)
    setTimeout(() => { setVisibleCount(prev => prev + 4); setLoadingMore(false) }, 800)
  }, [loadingMore])
  ...
  const toggleLike = (clip) => {
    setLiked(prev => prev.includes(clip.id) ? prev.filter(l => l !== clip.id) : [...prev, clip.id])
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <div className="max-w-6xl mx-auto px-8 pt-32 pb-32">
        ...
        <h1 className="font-black text-4xl md:text-5xl text-white mb-8" style={{ fontFamily: 'monospace' }}>Find Your Vibe 🎮</h1>

        <div className="relative mb-6">
          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">🔍</span>
          <input type="text" placeholder="Search by game, title or music..." value={search}
        ...
        <div className="flex gap-3 flex-wrap mb-10">
          {GAMES.map(game => (
            <button key={game} onClick={() => setActiveGame(game)}
              className={`px-4 py-2 rounded-lg text-xs font-bold tracking-widest transition-all duration-200 ${activeGame === game ? 'bg-gradient-to-r from-cyan-400 to-purple-500 text-black' : 'bg-[#0b1425] border border-cyan-500/20 text-slate-400 hover:border-cyan-400 hover:text-cyan-400'}`}
              style={{ fontFamily: 'monospace' }}>{game}</button>
          ))}
        </div>

        <p className="text-slate-500 text-sm mb-6">{filtered.length} clips found</p>

        {isLoading ? <SkeletonGrid count={8} /> : filtered.length > 0 ? (
          <>
