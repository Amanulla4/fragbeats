...
function Feed() {
  ...
  const handleScroll = useCallback(() => {
    ...
    // Increment view count for current clip
    const clip = clips[index]
    if (clip) {
      supabase.from('clips').update({ views: (clip.views || 0) + 1 }).eq('id', clip.id)
    }
  }, [clips])
              ...
              <div className="flex items-center gap-2">
                ...
                {user?.id !== clip.user_id && (
                  <button
                    ...
                    className={`ml-2 px-3 py-1 rounded text-xs font-black tracking-widest pointer-events-auto transition-all duration-200 ${
